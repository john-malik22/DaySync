...
// --- SPLITS FEATURE BACKEND ENDPOINTS ---
...
app.get('/api/splits', authenticate, (req, res) => {
  ...
  const userSplits = store.splits.filter(s =>
    ...
    (s.members || []).some(m => m.id === req.user.id)
  );
  res.json(userSplits);
});
...
app.post('/api/splits', authenticate, (req, res) => {
  ...
  console.log(`[SPLIT CREATED] "${newSplit.name}" created by ${req.user.name} with code ${newSplit.shareCode}`);
  res.json(newSplit);
});
...
app.get('/api/splits/preview/:code', authenticate, (req, res) => {
  ...
  const store = db.read();
  const split = (store.splits || []).find(s => s.shareCode && s.shareCode.toUpperCase() === cleanCode);

  if (!split) {
    return res.status(404).json({ error: "That Split code isn't valid." });
  }

  if (split.codeActive === false) {
    return res.status(400).json({ error: 'This Split code is no longer active.' });
  }

  const owner = (split.members || []).find(m => m.id === split.ownerId) || { name: 'Owner' };
  const isAlreadyMember = (split.members || []).some(m => m.id === req.user.id);
...
app.post('/api/splits/join', authenticate, (req, res) => {
  ...
  const isMember = split.members.some(m => m.id === req.user.id);

  if (isMember) {
    return res.status(400).json({ error: "You're already a member of this Split.", split });
  }
  ...
  db.write(store);
  console.log(`[SPLIT JOINED] User ${req.user.name} joined "${split.name}" via code ${cleanCode}`);

  res.json({
    success: true,
    message: `Joined ${split.name}.`,
    split
  });
});
...
app.post('/api/splits/:id/expenses', authenticate, (req, res) => {
  ...
  const newExpense = {
    id: `spexp_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
    title: title.trim(),
    amount: parseFloat(amount),
    paidBy: req.user.id,
    paidByName: req.user.name,
    splitWith: Array.isArray(splitWith) && splitWith.length > 0 ? splitWith : split.members.map(m => m.id),
    createdAt: new Date().toISOString()
  };

  split.expenses.push(newExpense);
  db.write(store);
  res.json(split);
});
...
app.post('/api/splits/:id/settle', authenticate, (req, res) => {
  const { toUserId, amount } = req.body;
  if (!toUserId || !amount) {
    return res.status(400).json({ error: 'Recipient user ID and settlement amount are required.' });
  }
  ...
  const toUser = (split.members || []).find(m => m.id === toUserId);
  if (!toUser) {
    return res.status(400).json({ error: 'Recipient is not a member of this Split group.' });
  }

  split.settlements = split.settlements || [];
  split.settlements.push({
    id: `spset_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
    from: req.user.id,
...
app.post('/api/splits/:id/regenerate-code', authenticate, (req, res) => {
  ...
  if (split.ownerId !== req.user.id) {
    return res.status(403).json({ error: 'Only the Split owner can regenerate the share code.' });
  }

  let newCode = generateSplitShareCode(split.name);
  while (store.splits.some(s => s.shareCode === newCode)) {
  ...
  console.log(`[SPLIT CODE REGENERATED] Split "${split.name}" new code: ${newCode}`);
  res.json({ success: true, shareCode: newCode, split });
});
