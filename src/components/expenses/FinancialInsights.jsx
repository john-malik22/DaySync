...
export function FinancialInsights() {
  ...
  return (
    <div className="glass-card">
        ...
        {!editingTarget ? (
          ...
          <form onSubmit={handleSaveTarget} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            <input
              ...
              placeholder="Target ₹"
              style={{ width: '100px', minHeight: '34px', padding: '4px 8px', fontSize: '0.84rem' }}
              autoFocus
            />
            <button type="submit" className="btn-primary" style={{ padding: '4px 10px', fontSize: '0.8rem', minHeight: '34px' }}>
              <Check size={14} /> Save
            </button>
          </form>
        )}
      ...
      {/* 3 Overview Cards */}
      <div className="grid-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 'var(--space-2)', marginBottom: 'var(--space-5)' }}>
        ...
        <div style={{ padding: '14px', borderRadius: 'var(--radius-md)', background: 'rgba(99, 102, 241, 0.1)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', marginBottom: 'var(--space-5)' }}>
        <div style={{
          ...
          border: '1px solid var(--border-color)', fontSize: '0.84rem'
        }}>
      ...
      <h4 style={{ marginBottom: 'var(--space-2)', fontSize: '0.88rem' }}>Category Breakdown</h4>
      {Object.keys(categories).length === 0 ? (
        <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>No financial transactions logged yet.</p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {Object.entries(categories).map(([cat, val]) => {
            const isInc = cat.startsWith('[Income]');
            return (
              <div key={cat} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                <span>{cat}</span>
